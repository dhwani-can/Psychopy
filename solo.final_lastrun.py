...
expName = 'solo.final'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\91982\\Desktop\\solo.final_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    welcomeroutine = data.Routine(
        name='welcomeroutine',
        components=[instructionText, start],
    )
    ...
    trials = data.TrialHandler2(
        ...
        extraInfo=expInfo, 
        originPath=-1, 
        trialList=data.importConditions('solo.final.xlsx'), 
        seed=None, 
    )
    ...
    for thisTrial in trials:
        ...
        # create an object to store info about Routine condition
        condition = data.Routine(
            name='condition',
        ...
        key_resp.rt = []
        _key_resp_allKeys = []
        # store start times for condition
        condition.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
        condition.tStart = globalClock.getTime(format='float')
        condition.status = STARTED
        thisExp.addData('condition.started', condition.tStart)
        condition.maxDuration = None
        # keep track of which components have finished
        conditionComponents = condition.components
        for thisComponent in condition.components:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        if isinstance(trials, data.TrialHandler2) and thisTrial.thisN != trials.thisTrial.thisN:
            continueRoutine = False
        condition.forceEnded = routineForceEnded = not continueRoutine
        while continueRoutine:
            # get current time
            t = routineTimer.getTime()
            tThisFlip = win.getFutureFlipTime(clock=routineTimer)
            ...
            # check if all components have finished
            if not continueRoutine:  # a component has requested a forced-end of Routine
                condition.forceEnded = routineForceEnded = True
                break
            continueRoutine = False  # will revert to True if at least one component still running
            for thisComponent in condition.components:
                if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
        ...
        for thisComponent in condition.components:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # store stop times for condition
        condition.tStop = globalClock.getTime(format='float')
        condition.tStopRefresh = tThisFlipGlobal
        thisExp.addData('condition.stopped', condition.tStop)
        # check responses
