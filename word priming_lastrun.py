...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\91982\\Desktop\\psychopy\\word priming\\word priming_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    # --- Initialize components for Routine "T" ---
    ...
    for thisTrial_2 in trials_2:
        ...
        # --- Prepare to start Routine "T" ---
        # create an object to store info about Routine T
        T = data.Routine(
            name='T',
        ...
        # store start times for T
        T.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
        T.tStart = globalClock.getTime(format='float')
        T.status = STARTED
        thisExp.addData('T.started', T.tStart)
        T.maxDuration = None
        # keep track of which components have finished
        TComponents = T.components
        for thisComponent in T.components:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        if isinstance(trials_2, data.TrialHandler2) and thisTrial_2.thisN != trials_2.thisTrial.thisN:
            continueRoutine = False
        T.forceEnded = routineForceEnded = not continueRoutine
        while continueRoutine and routineTimer.getTime() < 10.0:
            ...
            # check if all components have finished
            if not continueRoutine:  # a component has requested a forced-end of Routine
                T.forceEnded = routineForceEnded = True
                break
            continueRoutine = False  # will revert to True if at least one component still running
            for thisComponent in T.components:
                if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
        ...
        for thisComponent in T.components:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # store stop times for T
        T.tStop = globalClock.getTime(format='float')
        T.tStopRefresh = tThisFlipGlobal
        thisExp.addData('T.stopped', T.tStop)
        ...
        if T.maxDurationReached:
            routineTimer.addTime(-T.maxDuration)
        elif T.forceEnded:
            routineTimer.reset()
        else:
